...
const universities = graduates.map(universities => universities.university).sort();
...
for (student in graduates) {
  if (graduates[student].university.match('Uni')) {
    unisWithUni.push(graduates[student].university);
  }
}
...
*/
const lowPopulationAnimals = zooAnimals.filter(popLow => popLow.population < 5);
console.log(lowPopulationAnimals);

/* Request 4: .reduce()
The zoos need to know their total animal population across the United States. Find the total population from all the zoos using the .reduce() method. Remember the reduce method takes two arguments: a callback (which itself takes two args), and an initial value for the count.
*/
const populationTotal = zooAnimals.reduce(function(popTotal, item) {
  return popTotal + item.population;
}, 0);
